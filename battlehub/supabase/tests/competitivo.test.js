...
(async () => {
  ...
  const msg = th && await q1('select body from messages where thread_id = $1 order by id desc limit 1', [th.thread_id]);
  ...
  const firstMsg = await q1('select body, sender_id from messages where thread_id = $1 order by id limit 1', [cs.thread_id]);
  ...
  await refuse('outro jogador não manda vídeo', call(P[4], 'case_submit_video', { p_case: cs.id, p_path: P[4] + '/v.mp4', p_link: null }), /não encontrada/);
  await refuse('vídeo sem nada', call(sus, 'case_submit_video', { p_case: cs.id, p_path: null, p_link: ' ' }), /vídeo/);
  await refuse('arquivo de outra pasta', call(sus, 'case_submit_video', { p_case: cs.id, p_path: P[4] + '/v.mp4', p_link: null }), /inválido/);
  await refuse('link inválido', call(sus, 'case_submit_video', { p_case: cs.id, p_path: null, p_link: 'ftp://video' }), /https/);
  const sv = await must('suspeito manda o vídeo', call(sus, 'case_submit_video', { p_case: cs.id, p_path: sus + '/caso.mp4', p_link: 'https://drive.google.com/file/d/abc' }));
