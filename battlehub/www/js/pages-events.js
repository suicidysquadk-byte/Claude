...
(function () {
  ...
  function standingsTable(rows, e, showStatus) {
    ...
    return '<ol class="stand">' + rows.map((x, i) => '<li class="' + (x.status === 'eliminado' ? 'out' : '') + (q && i < q && e.status === 'andamento' && !showStatus ? ' in-zone' : '') + (e.my_entry && e.my_entry.id === x.entry_id ? ' me' : '') + '">' +
      ...
      '<span class="st-mem">' + (x.members || []).slice(0, 4).map((m) => U.av(m, 'xs')).join('') + '<small>' + x.drops + ' queda' + (x.drops === 1 ? '' : 's') + ' · ' + x.booyahs + ' Booyah' + (x.booyahs === 1 ? '' : 's') + ' · ' + U.plural(x.kills, 'abate', 'abates') + '</small></span></div>' +
  ...
  pages.event = async function (p) {
    ...
    return {
      html: '<section class="page event-page k-' + e.kind + '">' + BH.backRow() +
        ...
        (champ ? '<section class="champ-card"><span class="champ-crown">' + I('crown') + '</span><small>Campeões</small><h2>' + esc(champ.name) + '</h2><div class="champ-mem">' + (champ.members || []).map((m) => U.av(m, 'md')).join('') + '</div><p>' + U.plural(champ.points, 'ponto', 'pontos') + ' · ' + champ.booyahs + ' Booyah' + (champ.booyahs === 1 ? '' : 's') + ' · ' + U.plural(champ.kills, 'abate', 'abates') + '</p></section>' : '') +
        cta + admin +
