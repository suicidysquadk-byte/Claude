...
(function () {
    ...
    return {
      html: '<section class="page">' +
        ...
        (rows.length ? '<div class="podium" aria-label="Pódio de ' + esc(label) + '">' + podium.map((x) =>
          ...
          '<ol class="rank-list stagger">' + rows.slice(3).map((x) => '<li class="' + (x.me ? 'me' : '') + '"><button type="button" class="rank-row ripple' + (x.user.plate ? ' has-pl' : '') + '"' + (x.user.id ? ' data-act="profile" data-id="' + x.user.id + '"' : ' disabled') + '>' + U.plate(x.user) +
  ...
  pages.guild = async function (p) {
    ...
    return {
      html: '<section class="page">' + BH.backRow() +
        ...
        '<ul class="member-list stagger">' + g.members.map((m) => '<li><button type="button" class="rank-row ripple' + (m.plate ? ' has-pl' : '') + '" data-act="guildMember" data-id="' + m.id + '" data-guild="' + g.id + '">' + U.plate(m) + U.av(m, 'sm') +
  ...
  function personRow(p) {
    const f = p.friend;
    return '<li class="friend-row' + (p.plate ? ' has-pl' : '') + '">' + U.plate(p) + U.av(p, 'md') + '<button type="button" class="grow link-row" data-act="profile" data-id="' + p.id + '"><b>' + U.nick(p, { level: true, tag: true }) + '</b><small>#' + p.code + '</small></button>' +
